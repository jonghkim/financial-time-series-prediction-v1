...
### Module1.5 Lab1_Keras
#### Question1. 마지막 예제 mnist에서 처음 부분에 data import 하는 부분이 있습니다. data을 변환하고 train 하는 이유가 있을것 같아서 문의드립니다.  어떤 모델에서든 항상 0~1값으로 변경해야하는것인지?
...
#### Question2. 마지막 예제 mnist에서 원본 data의 각 몇%로 x_train, x_test에 각각 할당되었는지요? 할당도 황금(?) 비율이 있는지요? 그리고 예를 들어 data가 충분하지 않을때 Train에 많이 할당하면 좋을텐데, 이런 경우를 위해서 할당량을 조절할 방법이 있나요?

    현재 x_train.shape = (60000, 28, 28) / x_test.shape = (10000, 28, 28) 으로 train은 60000 개 test는 10000개로 쪼개져 있습니다. 
...
#### Question3. Build Model부분이 마치 black box 같아 보입니다. 모델 학습에도 변수들의 변화되는 과정을 print 해볼만한 방법이 있을까요? 
...
#### Question4. 텐서플로우에서 모델 저장하면 checkpoint 파일등 여러개가 만들어지잖아요. 케라스에서는 .h 파일 하나만 생기는겁니까? 혹시 텐서플로우의 .pb 파일에 대응하는 건가요?
...
#### Question5. MNIST 예제 할 때, 오늘 코드 강의에서처럼 모델 만들어서 트레이닝하는 부분을 with tf.device('/gpu:0') 아래에 인덴트해서 넣으면 CPU가 아닌 GPU로 돌아가는 게 맞는지요? 
